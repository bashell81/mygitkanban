...
def insertProjectUpdateInfo(remoteurl,addline,delline,locline,users,curbranch,ndays=30):
    ...
    qry_cmd = "select * from %s where remoteurl='%s' and curbranch='%s'" % (tablename,remoteurl,curbranch)
    ...
    if ret == 0:
        ...
        mysql_cmd = "UPDATE %s SET addline=%s,delline=%s,locline=%s,ts='%s',users ='%s' WHERE remoteurl='%s' and curbranch ='%s'" % (tablename,addline,delline,locline,now,users,remoteurl,curbranch)
